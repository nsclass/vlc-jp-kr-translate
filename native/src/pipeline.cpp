...
auto decode_and_load_model(const std::filesystem::path& input_path,
...

  {
    ...
    auto [decode_res, model_res] =
        folly::coro::blockingWait(folly::coro::collectAll(
            ...
            folly::coro::co_withExecutor(
                &executor, [&]() -> folly::coro::Task<Result<Transcriber>> {
  ...
  double total_audio_s = static_cast<double>(chunks.back().offset_ms +
                                             chunks.back().duration_ms()) /
                         1000.0;
...
auto transcribe_chunks(Transcriber& transcriber,
  ...
  for (size_t i = 0; i < chunks.size(); ++i) {
    auto result = transcriber.transcribe_chunk_with_state(state.get(),
                                                          chunks[i], n_threads);
    if (!result) {
      std::cerr << std::format("\n  chunk {} failed: {}\n", i, result.error());
      continue;
    ...
    if ((i + 1) % 10 == 0 || i + 1 == chunks.size()) {
      std::cerr << std::format("  transcribed {}/{} chunks\r", i + 1,
                               chunks.size());
    }
  }
...
auto run_pipeline(const PipelineConfig& config) -> Result<PipelineResult> {
  ...
  if (!std::filesystem::exists(config.input_path)) {
    return std::unexpected("Input file not found: " +
  ...
  std::cerr << std::format(
      ...
      stage1->chunks.size(), stage1->total_audio_seconds,
      stage1->elapsed_seconds);
